...
public class ButacasDAO extends BaseDAO {
    ...
    public List<ButacaDTO> getButacasOcupadasNoAnuladasPorLocalizacion(
        ...
        List<ButacaDTO> list = query.from(qButacaDTO).join(qButacaDTO.parSesion, qSesionDTO)
            ...
            .where(qSesionDTO.id.eq(idSesion).and(qLocalizacionDTO.codigo.eq(codigoLocalizacion))
    ...
    public List<ButacaDTO> getButacas(
        ...
        List<ButacaDTO> list = query.from(qButacaDTO).join(qButacaDTO.parSesion, qSesionDTO)
            .join(qButacaDTO.parLocalizacion, qLocalizacionDTO).leftJoin(qButacaDTO.parCompra, qCompraDTO).fetch()
            .where(qSesionDTO.id.eq(idSesion).and(qLocalizacionDTO.codigo.eq(codigoLocalizacion))).list(qButacaDTO);
    ...
    public List<Tuple> getButacas(long idSesion) {
        ...
        List<Tuple> list = query.from(qButacaDTO, qTarifaDTO).join(qButacaDTO.parSesion, qSesionDTO)
            .join(qButacaDTO.parLocalizacion, qLocalizacionDTO).fetch().leftJoin(qButacaDTO.parCompra, qCompraDTO)
            .fetch().where(qSesionDTO.id.eq(idSesion).and(qButacaDTO.tipo.castToNum(Long.class).eq(qTarifaDTO.id))
                .and(qCompraDTO.reserva.eq(false)).and(qCompraDTO.parAbonado.isNull()).and(qButacaDTO.parCompra.caducada.eq(false)))
    ...
    public boolean estaOcupada(
        ...
        List<ButacaDTO> list = query.from(qButacaDTO, qSesionDTO, qLocalizacionDTO).where(
            qButacaDTO.parSesion.id.eq(qSesionDTO.id).and(qSesionDTO.id.eq(idSesion))
                ...
                .and(qButacaDTO.fila.eq(fila)).and(qButacaDTO.numero.eq(numero))).list(qButacaDTO);
    ...
    public void reservaButacas(
        ...
        try {
            ...
            for (Butaca butaca : butacas) {
                ...
                if (butaca.getFila() == null && butaca.getNumero() == null && noHayButacasLibres(sesionDTO,
                    localizacionDTO))
            ...
            if (butacaActual != null && e.getCause() instanceof ConstraintViolationException)
                throw new ButacaOcupadaException(sesionId, butacaActual.getLocalizacion(), butacaActual.getFila(),
    ...
    @Transactional
    private boolean noHayButacasLibres(
        SesionDTO sesionDTO,
        LocalizacionDTO localizacionDTO
    ) {
        return getOcupadas(sesionDTO.getId(), localizacionDTO.getCodigo()) >= localizacionDTO.getTotalEntradas()
            .intValue();
    }
    ...
    public int getOcupadas(
        ...
        List<ButacaDTO> list = query.from(qButacaDTO).innerJoin(qButacaDTO.parSesion, qSesionDTO)
            .innerJoin(qButacaDTO.parLocalizacion, qLocalizacionDTO).where(
                qSesionDTO.id.eq(sesionId).and(qButacaDTO.anulada.eq(false))
    ...
    public List<Tuple> getButacasCompra(
        ...
        QTarifaDTO qTarifaDTO = QTarifaDTO.tarifaDTO;
        return getQueryButacasCompra(idCompra).orderBy(getSort(qButacaDTO, sortParameter)).offset(start).limit(limit)
    ...
    public List<ButacaDTO> getButacasNoAnuladas(Long idSesion) {
        ...
        JPAQuery query = new JPAQuery(entityManager);
        return query.from(qButacaDTO).join(qButacaDTO.parCompra, qCompraDTO).fetch().where(
            qButacaDTO.parSesion.id.eq(idSesion).and(qButacaDTO.anulada.eq(false).and(qCompraDTO.reserva.eq(false))))
